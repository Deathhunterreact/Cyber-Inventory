...
RegisterNetEvent('cyber-inventory:steal')
AddEventHandler('cyber-inventory:steal', function()
    ...
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        ...
        if IsEntityPlayingAnim(searchPlayerPed, 'mp_arresting', 'idle', 3) or IsEntityDead(searchPlayerPed) or GetEntityHealth(searchPlayerPed) <= 0 then
            ESX.TriggerServerCallback('cyber-inventory:getIdentifier', function(identifier)
...
RegisterNetEvent('cyber-inventory:search')
AddEventHandler('cyber-inventory:search', function()
    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        local searchPlayerPed = GetPlayerPed(closestPlayer)
...
Citizen.CreateThread(function(source, args, raw)
    while true do
        ...
        if IsControlJustPressed(0, 58) then
                ...
                TriggerEvent('cyber-inventory:steal')
			end
        end
    end
end)
...
Citizen.CreateThread(function(source, args, raw)
    while true do
        ...
        if IsControlJustPressed(0, 58) then
            if IsPedArmed(GetPlayerPed(-1), 4) then
			TriggerEvent('cyber-inventory:search')
            end
        end
    end
end)
...
RegisterNUICallback('StealCash', function(data)
    TriggerServerEvent('cyber-inventory:StealCash', data)
end)
RegisterNUICallback('SeizeCash', function(data)
    TriggerServerEvent('cyber-inventory:SeizeCash', data)
end)
