...
function closeInventory() {
    ...
    $.post("http://cyber-inventory/NUIFocusOff", JSON.stringify({}));
    $('#search').val('');
...
function secondInventorySetup(invOwner, items, invTier, money, plyId) {
    ...
    $('#other-used').html("Ağırlık: " + ikinciagirlik.toFixed(1));
}
...
$(document).ready(function () {
        ...
        if (event.shiftKey) {
            if (itemData.usable) {
                if (itemData.qua > 0) {
                    ...
                    $.post("http://cyber-inventory/UseItem", JSON.stringify({
    ...
    $('#give').click(function (event, ui) {
        if (draggingItem != null && dragging) {
            ...
            if (itemData.canRemove) {
                $.post("http://cyber-inventory/GetNearPlayers", JSON.stringify({
                    originItem: itemData,
                    action: 'give'
                }));
    ...
    $("#seize").mouseenter(function () {
        ...
        $.post("http://cyber-inventory/SeizeCash", JSON.stringify({
    ...
    $("#steal").mouseenter(function () {
        ...
        $.post("http://cyber-inventory/StealCash", JSON.stringify({
            target: destinationOwner
        }));
    });
    ...
    $('#drop').click(function (event, ui) {
        if (dragging) {
            ...
            if (itemData.canRemove) {
                ...
                $.post("http://cyber-inventory/DropItem", JSON.stringify({
                    item: itemData,
                    qty: dropCount
                }));
                successAudio.play();
       ...
       $('#inventoryOne, #inventoryTwo').on('mouseenter', '.slot', function () {
        ...
        if (itemData !== undefined) {
            ...
            if (itemData.description !== undefined) {   	 
    			if (qualityI) {
                	if (serialI) {
						$('.Information').find('.info-desc').html(itemData.description +'<br /> Serial number: ' + itemData.ser + '<br /> Durability: ' + Number(itemData.qua.toFixed(2)));
					} else {
						$('.Information').find('.info-desc').html(itemData.description + '<br /> Durability: ' + Number(itemData.qua.toFixed(2)));
					}
				} else {
					if (serialI) {
						$('.Information').find('.info-desc').html(itemData.description +'<br /> Serial number: ' + itemData.ser);
					} else {
						$('.Information').find('.info-desc').html(itemData.description);
					}
				}
            	...
            	if (qualityI) { 
					if (serialI) {
						$('.Information').find('.info-desc').html('The item does not have any information. <br /> Serial number: ' + itemData.ser + '<br /> Durability: ' + Number(itemData.qua.toFixed(1)));
					} else {
						$('.Information').find('.info-desc').html('The item does not have any information' + '<br /> Durability: ' + Number(itemData.qua.toFixed(1)));
					}
				} else {
					if (serialI) {
						$('.Information').find('.info-desc').html('The item does not have any information <br /> Seria number: ' + itemData.ser);
					} else {
...
$('.popup-body').on('click', '.cashstore', function () {
    $.post("http://cyber-inventory/CashStore", JSON.stringify({
        action: 'cashstore',
        item: $(this).data("id"),
...
$('.popup-body').on('click', '.cashtake', function () {
    $.post("http://cyber-inventory/CashTake", JSON.stringify({
        action: 'cashtake',
        item: $(this).data("id"),
        count: parseInt($("#count").val()),
        owner: destinationOwner,
...
function AttemptDropInEmptySlot(origin, destination, moveQty) {
    ...
    if (result === -1) {
        ...
        if (moveQty === item.qty) {
            ...
            $.post("http://cyber-inventory/MoveToEmpty", JSON.stringify({
            ...
            $.post("http://cyber-inventory/EmptySplitStack", JSON.stringify({
                originOwner: origin.parent().data('invOwner'),
                originSlot: origin.data('slot'),
                originTier: origin.parent().data('invTier'),
    ...
    if (result === -1) {
        ...
        if (originItem.itemId === destinationItem.itemId && destinationItem.stackable) {
            ...
            if (moveQty != originItem.qty && destinationItem.qty + moveQty <= destinationItem.max) {
                ...
                successAudio.play();
                $.post("http://cyber-inventory/SplitStack", JSON.stringify({
                    originOwner: origin.parent().data('invOwner'),
                    originTier: origin.parent().data('invTier'),
                ...
                if (destinationItem.qty === destinationItem.max) {
                    ...
                    successAudio.play();
                    $.post("http://cyber-inventory/SwapItems", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originItem: origin.find('.item').data('item'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                    ...
                    successAudio.play();
                    $.post("http://cyber-inventory/CombineStack", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                        originItem: originItem,
                    ...
                    $.post("http://cyber-inventory/TopoffStack", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                        originItem: originItem,
...
function AddItemToSlot(slot, data) {
    ...
    if (qualityI) {
        ...
        if (data.qua > 99) {
            ...
            slot.find('.item-qua').html("PERFECT")
            ...
            slot.find('.item-qua').html("BROKEN");
...
$('.popup-body').on('click', '.player', function () {
    ...
    if (action === "nearPlayersGive") {
        ...
        if (givingItem != null) {
            ...
            $.post("http://cyber-inventory/GiveItem", JSON.stringify({
                target: target,
        ...
        $.post("http://cyber-inventory/GiveCash", JSON.stringify({
            target: target,
            item: givingItem,
            count: count
        }), function (status) {
