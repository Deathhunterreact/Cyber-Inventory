...
loadedInventories = {}
local itemq = nil
local LastAddedItemSlot = nil

RegisterServerEvent('cyber-inventory:openInventory')
AddEventHandler('cyber-inventory:openInventory', function(inventory)
...
RegisterServerEvent('cyber-inventory:closeInventory')
AddEventHandler('cyber-inventory:closeInventory', function(inventory)
    if openInventory[inventory.owner] == nil then
        openInventory[inventory.owner] = {}
    end
...
RegisterServerEvent('cyber-inventory:refreshInventory')
AddEventHandler('cyber-inventory:refreshInventory', function(owner)
    if openInventory[owner] == nil then
        openInventory[owner] = {}
    end

    for k, v in pairs(openInventory[owner]) do
        TriggerClientEvent('cyber-inventory:refreshInventory', k)
    end
end)
...
RegisterServerEvent("cyber-inventory:MoveToEmpty")
AddEventHandler("cyber-inventory:MoveToEmpty", function(data)
    ...
    if data.originOwner == data.destinationOwner and data.originTier.name == data.destinationTier.name then
        ...
        originInvHandler.applyToInventory(data.originOwner, function(inventory)
            ...
            TriggerEvent('cyber-inventory:refreshInventory', data.originOwner)
        end)
    else
        local originInvHandler = InvType[data.originTier.name]
        ...
        if data.originTier.name == 'shop' then
            ...
            if player.getMoney() >= data.originItem.price * data.originItem.qty then
                ...
                TriggerClientEvent('cyber-inventory:refreshInventory', data.target)
                return
            end
        end
        ...
        if data.destinationTier.name == 'shop' then
            TriggerEvent('cyber-inventory:refreshInventory', data.originOwner)
            TriggerEvent('cyber-inventory:refreshInventory', data.destinationOwner)
            return
        end

        if data.destinationTier.name == 'bag' then
            if data.originItem.id == Config.BagItem then
                TriggerEvent('cyber-inventory:refreshInventory', data.originOwner)
                TriggerEvent('cyber-inventory:refreshInventory', data.destinationOwner)
                return
            end
        ...
        if data.destinationTier.name == 'drop' then
            if data.originItem.id == Config.BagItem then
                MySQL.Sync.execute("UPDATE `cyber_inventory` SET `drop`=@drop WHERE owner = @owner AND type = @type", { 
                    ['@owner'] = data.originItem.uni,
                    ['@type'] = 'bag',
                    ['@drop'] = '1'
                })
            end
        ...
        if data.originTier.name == 'drop' then
            if data.originItem.id == Config.BagItem then
                MySQL.Sync.execute("UPDATE `cyber_inventory` SET `drop`=@drop WHERE owner = @owner AND type = @type", { 
                    ['@owner'] = data.originItem.uni,
                    ['@type'] = 'bag',
        ...
        originInvHandler.applyToInventory(data.originOwner, function(originInventory)
            destinationInvHandler.applyToInventory(data.destinationOwner, function(destinationInventory)
                ...
                if data.originTier.name == 'player' then
                    ...
                    TriggerEvent('cyber-inventory:notifyImpendingRemoval', data.originItem, data.originItem.qty, ownerPlayer.source)
                ...
                if data.destinationTier.name == 'player' then
                    ...
                    TriggerEvent('cyber-inventory:notifyImpendingAddition', data.originItem, data.originItem.qty, destinationPlayer.source)
...
RegisterServerEvent("cyber-inventory:SwapItems")
        ...
        originInvHandler.applyToInventory(data.originOwner, function(originInventory)
            destinationInvHandler.applyToInventory(data.destinationOwner, function(destinationInventory)
                ...
                if data.originTier.name == 'player' then
                    ...
                    TriggerEvent('cyber-inventory:notifyImpendingAddition', data.originItem, data.originItem.qty, originPlayer.source)
                    TriggerEvent('cyber-inventory:notifyImpendingRemoval', data.destinationItem, data.destinationItem.qty, originPlayer.source)
                ...
                if data.destinationTier.name == 'player' then
                    ...
                    data.destinationItem.block = true
                    local destinationPlayer = ESX.GetPlayerFromIdentifier(data.destinationOwner)
                    TriggerEvent('cyber-inventory:notifyImpendingRemoval', data.originItem, data.originItem.qty, destinationPlayer.source)
                    TriggerEvent('cyber-inventory:notifyImpendingAddition', data.destinationItem, data.destinationItem.qty, destinationPlayer.source)
...
RegisterServerEvent("cyber-inventory:CombineStack")
        ...
        originInvHandler.applyToInventory(data.originOwner, function(originInventory)
            destinationInvHandler.applyToInventory(data.destinationOwner, function(destinationInventory)
                ...
                if data.originTier.name == 'player' then
                    data.originItem.block = true
                    local originPlayer = ESX.GetPlayerFromIdentifier(data.originOwner)
                    TriggerEvent('cyber-inventory:notifyImpendingRemoval', data.originItem, data.originItem.qty, originPlayer.source)
...
RegisterServerEvent("cyber-inventory:TopoffStack")
...
RegisterServerEvent("cyber-inventory:EmptySplitStack")
        ...
        originInvHandler.applyToInventory(data.originOwner, function(originInventory)
            destinationInvHandler.applyToInventory(data.destinationOwner, function(destinationInventory)
                ...
                if data.originTier.name == 'player' then
                    local originPlayer = ESX.GetPlayerFromIdentifier(data.originOwner)
                    data.originItem.block = true
                    TriggerEvent('cyber-inventory:notifyImpendingRemoval', data.originItem, data.moveQty, originPlayer.source)
                ...
                if data.destinationTier.name == 'player' then
                    local destinationPlayer = ESX.GetPlayerFromIdentifier(data.destinationOwner)
                    data.originItem.block = true
                    TriggerEvent('cyber-inventory:notifyImpendingAddition', data.originItem, data.moveQty, destinationPlayer.source)
...
RegisterServerEvent("cyber-inventory:SplitStack")
AddEventHandler("cyber-inventory:SplitStack", function(data)
    ...
    if data.originOwner == data.destinationOwner and data.originTier.name == data.destinationTier.name then
        ...
        originInvHandler.applyToInventory(data.originOwner, function(inventory)
            ...
            inventory[tostring(data.destinationSlot)].quality = inventory[tostring(data.destinationSlot)].quality
            TriggerEvent('cyber-inventory:refreshInventory', data.originOwner)
        end)
    else
        local originInvHandler = InvType[data.originTier.name]
...
RegisterServerEvent("cyber-inventory:GiveItem")
AddEventHandler("cyber-inventory:GiveItem", function(data)
    ...
    if targetPlayer.canCarryItem(data.originItem.id, data.count) then
        ...
        if data.originTier.name == 'bag' then
            TriggerClientEvent('cyber-inventory:refreshInventory', source)
            TriggerClientEvent('cyber-inventory:refreshInventory', data.target)
            return
        end
		...
		TriggerEvent('cyber-inventory:notifyImpendingRemoval', data.originItem, data.count, source)
		TriggerEvent('cyber-inventory:notifyImpendingAddition', data.originItem, data.count, data.target)
		local targetPlayer = ESX.GetPlayerFromId(data.target)
		...
		TriggerClientEvent('cyber-inventory:refreshInventory', source)
		TriggerClientEvent('cyber-inventory:refreshInventory', data.target)
		local msg = "Eşya verme, Eşyayı veren;\n" ..sourcePlayer.identifier .. "\n\nEşyayı Alan;\n" .. targetPlayer.identifier
		TriggerEvent('LquensLog:Send', msg)
	else
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Karşındaki bunu taşıyamaz', lenght = 4000 })
	end
end)

RegisterServerEvent("cyber-inventory:GiveCash")
AddEventHandler("cyber-inventory:GiveCash", function(data)
    local sourcePlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent("cyber-inventory:CashStore")
AddEventHandler("cyber-inventory:CashStore", function(data)
    local sourcePlayer = ESX.GetPlayerFromId(source)
    if data.item == 'cash' then
        if sourcePlayer.getMoney() >= data.count then
            sourcePlayer.removeMoney(data.count)
            ...
            invHandler.applyToInventory(data.owner, function(inventory)
                ...
                TriggerEvent('cyber-inventory:refreshInventory', sourcePlayer.identifier)
                TriggerEvent('cyber-inventory:refreshInventory', data.owner)
            end)
        end
    elseif data.item == 'black_money' then
        if sourcePlayer.getAccount('black_money').money >= data.count then
...
RegisterServerEvent("cyber-inventory:CashTake")
AddEventHandler("cyber-inventory:CashTake", function(data)
    local sourcePlayer = ESX.GetPlayerFromId(source)
    if data.item == 'cash' then
        local invHandler = InvType[data.destinationTier.name]
        invHandler.applyToInventory(data.owner, function(inventory)
...
function addToInventory(item, type, inventory)
    local max = getItemDataProperty(item.name, 'max') or 9999999999
...
function GetLastAddedItemSlot()
    Citizen.Wait(5)
    return LastAddedItemSlot
end

ESX.RegisterServerCallback('cyber-inventory:GetLastAddedItemSlot', function(source, cb)
    local slot = GetLastAddedItemSlot()
    cb(slot)
end)

ESX.RegisterServerCallback('cyber-inventory:getSecondaryInventory', function(source, cb, type, identifier)
...
function createItem(name, count, quality, serialnumber, uniq)
    return { name = name, count = count, quality = quality, serialnumber = serialnumber, uniq = uniq}
end

ESX.RegisterServerCallback('cyber-inventory:canOpenInventory', function(source, cb, type, identifier)
...
RegisterNetEvent('cyber-inventory:saveInventory')
AddEventHandler('cyber-inventory:saveInventory', function()
     saveInventoriesuni()
end)

function saveLoadedInventory(identifier, type, data)
    if table.length(data) > 0 then
        MySQL.Async.execute('UPDATE cyber_inventory SET data = @data WHERE owner = @owner AND type = @type', {
            ['@owner'] = identifier,
            ['@type'] = type,
            ...
            TriggerEvent('cyber-inventory:savedInventory', identifier, type, data)
        end)
    end
end

function createInventory(identifier, type, data)
    MySQL.Async.execute('INSERT INTO cyber_inventory (owner, type, data) VALUES (@owner, @type, @data)', {
        ['@owner'] = identifier,
        ['@type'] = type,
        ['@data'] = json.encode(data)
    }, function()
        TriggerEvent('cyber-inventory:createdInventory', identifier, type, data)
    end)
end

function deleteInventory(identifier, type)
    MySQL.Async.execute('DELETE FROM cyber_inventory WHERE owner = @owner AND type = @type', {
        ['@owner'] = identifier,
        ['@type'] = type
    }, function()
        TriggerEvent('cyber-inventory:deletedInventory', identifier, type)
    end)
end
...
function applyToInventory(identifier, type, f)
    ...
    if loadedInventories[type][identifier] and table.length(loadedInventories[type][identifier]) > 0 then
        TriggerEvent('cyber-inventory:modifiedInventory', identifier, type, loadedInventories[type][identifier])
    else
        TriggerEvent('cyber-inventory:modifiedInventory', identifier, type, nil)
    end
end

function loadInventory(identifier, type, cb)
    MySQL.Async.fetchAll('SELECT data FROM cyber_inventory WHERE owner = @owner and type = @type', {
        ['@owner'] = identifier,
        ['@type'] = type
    }, function(result)
        if #result == 0 then
        ...
        cb(inventory)
        TriggerEvent('cyber-inventory:loadedInventory', identifier, type, inventory)
    end)
end
    ...
    if isWeapon(data.originItem.id) then
        if data.originOwner == data.destinationOwner and data.originTier.name == data.destinationTier.name then
            if data.destinationSlot > 5 then
                TriggerClientEvent('cyber-inventory:removeCurrentWeapon', source)
            end
        else
            TriggerClientEvent('cyber-inventory:removeCurrentWeapon', source)
        end
