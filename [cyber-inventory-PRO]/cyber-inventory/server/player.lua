...
ESX.RegisterServerCallback('cyber-inventory:getPlayerInventory', function(source, cb)
    getPlayerDisplayInventory(ESX.GetPlayerFromId(source).identifier, cb)
end)

ESX.RegisterServerCallback('cyber-inventory:getCanDownQ', function(source, cb)
    cb(itemQlist)
end)
AddEventHandler('onResourceStart', function(resourceName)
    ...
    itemSlist = Config.CyberLicence
    itemUlist = Config.ItemUniqList
end)

ESX.RegisterServerCallback('cyber-inventory:CheckSlot', function(source, cb, slot)
...
RegisterServerEvent('cyber-inventory:decrase')
AddEventHandler('cyber-inventory:decrase', function(slot, item)
    local player = ESX.GetPlayerFromId(source)
    if player ~= nil then
...
RegisterServerEvent('cyber-inventory:decrasehand')
AddEventHandler('cyber-inventory:decrasehand', function(decreaseun ,slot)
    local player = ESX.GetPlayerFromId(source)
    if player ~= nil then
        local originInvHandler = InvType['player']
        InvType['player'].getInventory(player.identifier, function(inventory)
...
---------
RegisterServerEvent('cyber-inventory:repair')
AddEventHandler('cyber-inventory:repair', function(slot)
    local player = ESX.GetPlayerFromId(source)
    local originInvHandler = InvType['player']
    InvType['player'].getInventory(player.identifier, function(inventory)
        if itemQlist[inventory[tostring(slot)].name] ~= nil then
            originInvHandler.applyToInventory(player.identifier, function(inventory2)
                ...
                TriggerEvent('cyber-inventory:discord_log', player, dctext)
            end)
        else
...
RegisterServerEvent('cyber-inventory:lowPerc')
AddEventHandler('cyber-inventory:lowPerc', function()
...
RegisterServerEvent('cyber-inventory:lowPercS')
AddEventHandler('cyber-inventory:lowPercS', function()
    for k, v in pairs(Config.DurabilityInvs) do
        local loopv2 = MySQL.Sync.fetchAll("SELECT * FROM cyber_inventory WHERE cyber_inventory.type = @type ", {
            ['@type'] = v.name
        })
        local loopv1 = MySQL.Sync.fetchAll("SELECT COUNT(*) AS ToplamDeger FROM cyber_inventory WHERE cyber_inventory.type = @type ", {
            ['@type'] = v.name
        })
...
function DecresaByTime()
    TriggerEvent("cyber-inventory:lowPerc")
    TriggerEvent("cyber-inventory:lowPercS")
end

TriggerEvent('cyber-inventory:timer', 23, 59, DecresaByTime)

RegisterServerEvent('cyber-inventory:deleteSerial')
AddEventHandler('cyber-inventory:deleteSerial', function(slot, item)
    local player = ESX.GetPlayerFromId(source)
    local originInvHandler = InvType['player']
    InvType['player'].getInventory(player.identifier, function(inventory)
...
ESX.RegisterServerCallback('cyber-inventory:writeuniq', function(source, cb, slot)
    local player = ESX.GetPlayerFromId(source)
    local originInvHandler = InvType['player']
    InvType['player'].getInventory(player.identifier, function(inventory)
        originInvHandler.applyToInventory(player.identifier, function(inventory2)
...
RegisterServerEvent('cyber-inventory:writeuniqhand')
AddEventHandler('cyber-inventory:writeuniqhand', function(slot, uniqu)
    local player = ESX.GetPlayerFromId(source)
    local originInvHandler = InvType['player']
    InvType['player'].getInventory(player.identifier, function(inventory)
        if itemUlist[inventory2[tostring(slot)].name] ~= nil then
...
ESX.RegisterServerCallback('cyber-inventory:LastSlot', function(source, cb, slot)
    local player = ESX.GetPlayerFromId(source)
    if slot ~= nil then
        MySQL.Sync.execute("UPDATE `cyber_inventory` SET `slot`=@slot WHERE owner = @identifier", { 
...
ESX.RegisterServerCallback('cyber-inventory:GetLastSlot', function(source, cb)
    local player = ESX.GetPlayerFromId(source)
    MySQL.Async.fetchAll('SELECT slot FROM cyber_inventory WHERE owner = @owner', {
...
ESX.RegisterServerCallback('cyber-inventory:getUniq', function(source, cb, slot)
    local player = ESX.GetPlayerFromId(source)
    local originInvHandler = InvType['player']
    TriggerEvent('cyber-inventory:saveInventory')
    InvType['player'].getInventory(player.identifier, function(inventory)
        originInvHandler.applyToInventory(player.identifier, function(inventory2)
            if itemUlist[inventory2[tostring(slot)].name] ~= nil then
...
ESX.RegisterServerCallback('cyber-inventory:getItemName', function(source, cb, slot)
    local player = ESX.GetPlayerFromId(source)
    local originInvHandler = InvType['player']
    InvType['player'].getInventory(player.identifier, function(inventory)
        originInvHandler.applyToInventory(player.identifier, function(inventory2)
...
function ensurePlayerInventory(player)
    loadInventory(player.identifier, 'player', function()
        TriggerClientEvent('cyber-inventory:refreshInventory', player.source)
    end)
end
...
RegisterServerEvent('cyber-inventory:notifyImpendingRemoval')
AddEventHandler('cyber-inventory:notifyImpendingRemoval', function(item, count, playerSource)
...
RegisterServerEvent('cyber-inventory:notifyImpendingAddition')
AddEventHandler('cyber-inventory:notifyImpendingAddition', function(item, count, playerSource)
    local _source = playerSource or source
...
AddEventHandler('esx:onRemoveInventoryItem', function(source, item, count)
    local player = ESX.GetPlayerFromId(source)
    TriggerClientEvent('cyber-inventory:showItemUse', source, {
...
AddEventHandler('esx:onAddInventoryItem', function(source, esxItem, count)
    ...
    applyToInventory(player.identifier, 'player', function(inventory)
        ...
        if esxItem.name == Config.BagItem then
            MySQL.Sync.execute("INSERT INTO `cyber_inventory`(`owner`, `type`, `data`, `drop`) VALUES (@owner,@type,@data,@drop)", { 
                ['@owner'] = uniq,
                ['@type'] = 'bag',
                ['@data'] = '{}',
                ['@drop'] = '0'
...
RegisterNetEvent('cyber-inventory:discord_log')
AddEventHandler('cyber-inventory:discord_log', function(xPlayer, text)
